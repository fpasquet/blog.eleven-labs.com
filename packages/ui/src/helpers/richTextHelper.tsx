...
const getVariantReminderByAdmonitionValue = (
  variantValue: string
): undefined | VariantReminderType => {
  switch (variantValue) {
    case 'note':
      return 'note';
    case 'abstract':
...
export const getReminderByDomNode = (
  ...
  const variantValue = domNode?.attribs?.class
    ?.replace('admonition', '')
    .trim();
  ...
  const { title, content } = domNode.childNodes.reduce<{
    ...
    (result, childNode) => {
      if (
        childNode instanceof HTMLReactParser.Element &&
        childNode.attribs?.class?.match('admonition')
      ) {
  ...
  return (
    <Reminder
      my={{ xs: 'm' }}
      variant={variant as VariantReminderType}
      title={title}
    >
